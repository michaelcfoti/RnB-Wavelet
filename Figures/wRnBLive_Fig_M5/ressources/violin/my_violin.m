...
for i=i:size(Y,2)
    if isempty(lc) == 1
        ...
        if setX == 0
            h(i)=fill([F(:,i)+i;flipud(i-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc(i,:),'lineWidth',1);
        else
            h(i)=fill([F(:,i)+x(i);flipud(x(i)-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc(i,:),'lineWidth',1);
        end
    end
    hold on
    if setX == 0
